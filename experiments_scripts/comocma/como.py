...
import os
from comocma.sofomore_logger import SofomoreDataLogger
import random
...
class Sofomore(interfaces.OOOptimizer):
    ...
    def __init__(self,
        ...
        defopts = {'archive': True, 'restart': None, 'p_factor': 0.95, 'verb_filenameprefix': 'outsofomore' + os.sep,
                   'verb_log': 1, 'verb_disp': 100, 'update_order': sort_random, 'deque_maxlen': 3,
                   'adapt_minperc': 0.05,
        ...
        self.p_factor = self.opts['p_factor']
        self.deque_maxlen = self.opts['deque_maxlen']
        self.adapt_minperc = self.opts['adapt_minperc']
        ...
        self.feasible_archive = BiobjectiveNondominatedSortedList(reference_point=self.reference_point)
        self.restart_type = None
        self.restart_history = [None] * len(list_of_solvers_instances)
        self.kernel_starting_hypervolumes = [0] * len(list_of_solvers_instances)
        self.best_restart_previous = deque(maxlen=self.deque_maxlen)
        self.random_restart_previous = deque(maxlen=self.deque_maxlen)
    ...
    def tell(self, solutions, objective_values, constraint_values=None):
        """
        ...
        if constraint_values is not None:
            for x, f, g in zip(solutions, objective_values, constraint_values):
                if sum(np.maximum(0, g)) <= 0:
                    self.feasible_archive.add(f, info={'x': x})
        else:
            for x, f in zip(solutions, objective_values):
                self.feasible_archive.add(f, info={'x': x})
        ...
        if self.al is None and constraint_values is not None:
            self.al = AugmentedLagrangian(self.dimension)
        ...
        for ikernel, offspring in self.offspring:
            front_observed = self.NDA(
                [self.kernels[i].objective_values for i in range(self.num_kernels) if i != ikernel],
                reference_point=self.reference_point)

            hypervolume_improvements = [-float(front_observed.hypervolume_improvement(point))
                                        for point in objective_values[start:start + len(offspring)]]
            kernel = self.kernels[ikernel]

            if constraint_values is None:
                kernel.tell(offspring, hypervolume_improvements)
            else:
                self.al.set_coefficients(hypervolume_improvements, constraint_values)
                self.al.update(np.mean(hypervolume_improvements), np.mean(constraint_values, axis=0))

                kernel.tell(offspring, [uhvi + sum(self.al(g))
                                        for uhvi, g in zip(hypervolume_improvements,
                                                           constraint_values[start:start + len(offspring)])])
            ...
            if kernel.stop():
                ...
                if self.restart is not None:
                    run_improvement = self.feasible_archive.hypervolume - self.kernel_starting_hypervolumes[ikernel]
                    if self.restart_history[ikernel] == 'best':
                        self.best_restart_previous.append(run_improvement / kernel.countevals)
                    elif self.restart_history[ikernel] == 'random':
                        self.random_restart_previous.append(run_improvement / kernel.countevals)

                    kernel_to_add = self.restart(self)
                    self._told_indices += [self.num_kernels]
                    self.add(kernel_to_add)

                    self.restart_history.append(self.restart_type)
                    self.kernel_starting_hypervolumes.append(self.feasible_archive.hypervolume)
        ...
        if epsilon:
            self.epsilon_hypervolume_pareto_front = min(self.epsilon_hypervolume_pareto_front, epsilon)
        self.best_hypervolume_pareto_front = max(self.best_hypervolume_pareto_front, current_hypervolume)
    ...
    def pareto_front(self):
        ...
        """
        return self.NDA([kernel.objective_values for kernel in self.kernels if kernel.objective_values is not None],
                ...
                try:
                    print(' '.join((repr(self.countiter).rjust(5),
                                    ...
                                    '%4.1e' % (np.median([kernel.D.max() / kernel.D.min()
                                                          if not kernel.opts['CMA_diagonal'] or kernel.countiter >
                                                             kernel.opts['CMA_diagonal']
...
def best_chv_restart_kernel_fast(moes, sigma_factor=2, **kwargs):
    """create a kernel (solver) of TYPE CmaKernel by duplicating the kernel with
    best uncrowded hypervolume improvement.
    ...
    **kwargs :
    ...
    kernel_objective_values = [moes.kernels[i].objective_values for i in range(moes.num_kernels)]

    hvc = []
    for idx in range(moes.num_kernels):
        filtered_objective_values = [kernel_objective_values[i] for i in range(moes.num_kernels) if i != idx]
        front = moes.NDA(filtered_objective_values, moes.reference_point)
        f_pair = kernel_objective_values[idx]
...
def random_kernel_restart_kernel(moes, sigma_factor=2, **kwargs):
    """create a kernel (solver) of TYPE CmaKernel by duplicating a random kernel.
    ...
    **kwargs :
    ...
    Returns
    -------
    A kernel (solver) of TYPE CmaKernel derived from a random kernel.

    """

    idx = np.random.randint(0, len(list(moes)))

    ker = moes.kernels[idx]
    new_sigma0 = sigma_factor * ker.sigma
    if moes.opts['continue_stopped_kernel'] and idx not in moes._active_indices:
        moes._active_indices.append(idx)
        ker.sigma = new_sigma0
        ker.countiter = 0  # a hack before we find a better way to initialize the tolfunrel functionality
...
def stochastic_restart_kernel(moes, sigma_factor=2, x0_fct=None, sigma0=None, opts={}, **kwargs):
    """Generate unfairly, via a stochastic scenario, either a kernel via `best_chv_restart_kernel`
    or a kernel via `random_restart_kernel`.
...
def adaptive_restart_kernel(moes, sigma_factor=2, x0_fct=None, sigma0=None, opts={}, **kwargs):
    if moes.restart_type is None:
        moes.restart_type = 'best' if random.random() < 0.5 else 'random'

    elif len(moes.best_restart_previous) == 0 or \
            moes._number_of_calls_best_chv_restart < moes.adapt_minperc * moes._number_of_calls_random_restart:
        moes.restart_type = 'best'

    elif len(moes.random_restart_previous) == 0 or \
            moes._number_of_calls_random_restart < moes.adapt_minperc * moes._number_of_calls_best_chv_restart:
        moes.restart_type = 'random'

    else:
        best_restart_average = np.mean(moes.best_restart_previous)
        random_restart_average = np.mean(moes.random_restart_previous)
        moes.restart_type = 'best' if best_restart_average > random_restart_average else 'random'

    if moes.restart_type == 'best':
        moes._number_of_calls_best_chv_restart += 1
        return best_chv_restart_kernel(moes, sigma_factor, **kwargs)
...
def get_cmas(x_starts, sigma_starts, inopts=None, number_created_kernels=0):
    ...
    for i in range(len(x_starts)):
        try:
            bounds_transform = BoundTransform(list_of_opts[i]['bounds'])
